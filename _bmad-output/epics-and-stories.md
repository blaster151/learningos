...
**Implementation Notes:** AI-first architecture — learning happens through chat rather than static content pages. Objective mastery is now verified via quiz (4-question quiz per objective: MC, T/F, MC, Short Answer). AI assesses when objectives are "ready to quiz" during conversation, then learners take inline quizzes to confirm mastery (≥3/4 to pass). 3-state objective pills in the UI: ○ not ready (gray), 🧪 ready to quiz (amber, clickable), ✅ mastered (green).
...
### E5-S6: Objective Quiz System ✅

**As a** learner  
**I want to** verify my understanding through quizzes before objectives are marked complete  
**So that** mastery is validated rather than self-assessed

**Acceptance Criteria:**
- [x] Given the AI detects sufficient coverage of an objective in conversation, when it assesses objectives, then the objective is marked "ready to quiz" (🧪 amber pill)
- [x] Given I click a "ready to quiz" objective pill, when the quiz loads, then I see a 4-question inline quiz (MC → T/F → MC → Short Answer)
- [x] Given I answer all 4 questions, when quiz completes, then MC/TF are auto-graded and short answer is AI-graded via `/api/quiz/grade-essay`
- [x] Given I score ≥3/4, when results display, then the objective is marked ✅ mastered (green pill)
- [x] Given I score <3/4, when results display, then I see feedback and can retry
- [x] Given I am mid-quiz, when I want to cancel, then I can exit without losing chat context

**Story Points:** 8  
**Priority:** P0-Critical  
**Dependencies:** E5-S2, E3-S2  
**UX Reference:** Section 7 - Chat Mode (Objective Quiz)  
**API Reference:** POST /api/quiz/generate, POST /api/quiz/grade-essay  
**Implementation Notes:** Quiz questions generated by GPT-4 per objective. 3-state objective pills replace manual checkboxes. Assessment prompt updated to detect "ready to quiz" rather than auto-completing.

---

### E5-S7: Smart Bold Terms ✅

**As a** learner  
**I want to** see important domain terms bolded in AI responses  
**So that** I can identify and click key terminology to learn more

**Acceptance Criteria:**
- [x] Given the AI responds, when I read the message, then all domain-relevant terms are **bolded** (3–8 per message)
- [x] Given I click a bolded term, when the action fires, then it sends "Tell me about {term}" as my next message
- [x] Given I have previously learned concepts, when the AI generates a response, then it bolds those known terms consistently
- [x] Given the system prompt is built, when concepts are fetched, then up to 50 user concepts are injected for consistent bolding

**Story Points:** 3  
**Priority:** P1-High  
**Dependencies:** E5-S2, E6-S1  
**UX Reference:** Section 7 - Chat Mode (Bold Terms)  
**API Reference:** POST /api/chat (system prompt enhancement)  
**Implementation Notes:** System prompt expanded from 1 line to 6 lines of bolding instructions. User's known concepts fetched from Firestore and injected into prompt context.
...
**As a** learner  
**I want to** break down dense AI responses into smaller, expanded parts  
**So that** I can process complex explanations at my own pace

**Acceptance Criteria:**
- [x] Given an AI response is displayed, when I click "🔬 Unpack this" in Quick Actions, then the API splits it into 2–3 expanded chunks
- [x] Given unpacking completes, when I see results, then the original message collapses to "📦 Unpacked into N parts"
- [x] Given chunks are ready, when I see the first chunk, then I see a "Next part (2 of 3)" button to progressively reveal
- [x] Given all chunks are revealed, when I want the original, then I can click "show original" to collapse chunks and restore
- [x] Given chunks are displayed, when I see them, then they have numbered indigo-bordered bubbles distinct from regular messages

**Story Points:** 5  
**Priority:** P1-High  
**Dependencies:** E5-S2  
**UX Reference:** Section 7 - Chat Mode (Unpack)  
**API Reference:** POST /api/chat/unpack  
**Implementation Notes:** Uses GPT-4 to split content into chunks with expanded explanations. Crossfade animations between original and unpacked states.

---

### E5-S9: Continue Milestone ✅

**As a** learner  
**I want to** get back on track during a milestone chat after going on a tangent  
**So that** I can resume structured learning without losing context

**Acceptance Criteria:**
- [x] Given I am in a milestone chat session, when I see Quick Actions, then I see a "📍 Continue milestone" button
- [x] Given I click "Continue milestone", when the message sends, then it lists remaining uncompleted objectives and asks the AI to pick up with the next one
- [x] Given I am NOT in a milestone chat, when I see Quick Actions, then the "Continue milestone" button is hidden

**Story Points:** 2  
**Priority:** P1-High  
**Dependencies:** E5-S2, E3-S2  
**UX Reference:** Section 7 - Chat Mode (Quick Actions)  
**API Reference:** POST /api/chat (message with objective context)  
**Implementation Notes:** QuickActions component receives `hasMilestone` prop. Handler constructs prompt from uncompleted objectives array.

---

### E5-S10: Quick Actions Bar ✅

**As a** learner  
**I want to** have contextual action buttons after each AI response  
**So that** I can quickly interact without typing

**Acceptance Criteria:**
- [x] Given an AI response is displayed, when I see Quick Actions, then I see: 💡 Explain more, 📝 Example, ❓ Quiz me, 🎯 Simplify, 🔬 Unpack this
- [x] Given I am in a milestone chat, when I see Quick Actions, then I also see 📍 Continue milestone
- [x] Given I click any Quick Action, when it fires, then the corresponding action is triggered (send message, call API, or open UI)
