...
describe('Paths API Routes', () => {
  ...
  describe('POST /api/paths/scope-analyze', () => {
    it('should analyze scope for a goal', async () => {
      ...
      const request = new NextRequest('http://localhost:3000/api/paths/scope-analyze', {
        method: 'POST',
        headers: {
          'content-type': 'application/json',
        },
        body: JSON.stringify({ userId: 'user-123', goal: 'Learn React Hooks' }),
      });
  ...
  describe('POST /api/paths/narrow-suggest', () => {
    it('should return narrowing suggestions', async () => {
      ...
      vi.mocked(narrowSuggestLib.suggestNarrowTopics).mockResolvedValue([
        { title: 'React Hooks Fundamentals', description: 'Core hooks and mental model.', order: 1 },
        { title: 'State Management Patterns', description: 'When to use context, reducers, and libraries.', order: 2 },
        { title: 'Custom Hooks Workshop', description: 'Build reusable abstractions with hooks.', order: 3 },
      ] as any);

      const request = new NextRequest('http://localhost:3000/api/paths/narrow-suggest', {
        method: 'POST',
        headers: { 'content-type': 'application/json' },
        body: JSON.stringify({ userId: 'user-123', goal: 'Front-end web development' }),
      });
  ...
  describe('POST /api/paths/calibration/wave-1', () => {
    it('should return calibration pills for a goal', async () => {
      ...
      const request = new NextRequest('http://localhost:3000/api/paths/calibration/wave-1', {
        method: 'POST',
        headers: { 'content-type': 'application/json' },
        body: JSON.stringify({ userId: 'user-123', goal: 'Learn React Hooks' }),
      });
  ...
  describe('POST /api/paths/calibration/wave-2', () => {
    it('should return wave-2 pills when needed', async () => {
      ...
      const request = new NextRequest('http://localhost:3000/api/paths/calibration/wave-2', {
        method: 'POST',
        headers: { 'content-type': 'application/json' },
        body: JSON.stringify({
          userId: 'user-123',
    ...
    it('should require goal parameter', async () => {
      const request = new NextRequest('http://localhost:3000/api/paths/calibration/wave-2', {
        method: 'POST',
        headers: { 'content-type': 'application/json' },
        body: JSON.stringify({ userId: 'user-123', wave1Pills: [{ concept: 'x', reason: 'y' }] }),
      });
